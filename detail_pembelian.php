...
<input type="text" name="PenjualanID" value="<?php
echo $d['PenjualanID']; ?>" hidden>
<input type="text" name="PelangganID" value="<?php
echo $d['PelangganID']; ?>" hidden>
...
while($d detailpenjualan=mysqli_fetch_array ($detailpenjualan)) {
?>
<?php
if ($d detailpenjualan ['PenjualanID']== $d['PenjualanID']) {?>
<tr>
<td><?php echo $nos++; ?></td>
<td>
...
<input type="text" name="DetailID" value="<?php echo $d detailpenjualan['DetailID']; ?>" hidden>
<select name="Produk ID" class="form-control"onchange="this.form.submit()"> 
<option>--- Pilih Produk ---</option>
<?php include'../konekai.php';
$no =1;
$produk mysqli_query( Skoneksi, "SELECT*FROM produk");
while($d produk mysqli_fetch_array($produk)) {
?>
<option value="<?php echo $d_produk ['ProdukID']; ?>" <?php if ($d_produk['ProdukID']==$d_detailpenjualan['ProdukID']) { echo "selected"; } ?>> <?php echo $d_Produk['NamaProduk']; ?>
</option>
<?php } ?>
</select>
</div>
...
<input type="number" name= "JumlahProduk" value="<?php echo $d_detailpenjualan [ 'JumlahProduk']; ?>" class= "form-control">
          ...
          <td>
          <div class="form-group">
            <?php echo $d_detailpenjualan['Subtotal']; ?>
          </div>
          </td>
          <td>
            <div clasws="form-group">
...
<form method="post" action="simpan_total_harga.php">
    <?php
    include '../koneksi.php';
    TotalHarga FROM detailpenjualan WHERE  PenjualanID='$d [penjualanID]'");
    $row = mysqli_fetch assoc ($detailpenjualan);
$sum =$row['Totallarga'];
?>
<div class="row">
<div class="col-sm-10">
